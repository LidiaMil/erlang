...
-module(bin_tree).
-export([empty/0, node/3, is_tree/1, is_empty/1, is_node/1, data/1, left/1, right/1, view/1, split/2]).
...
split(empty, _) ->
    {empty, empty};
split({node, Data, Left, Right}, X) when Data < X ->
    case view(split(Right, X)) of
        {empty, GT} -> {node(Data, Left, empty), GT};
        {GTLeft, GTRight} -> {node(Data, Left, GTLeft), GTRight}
    end;
split({node, Data, Left, Right}, X) ->
    case view(split(Left, X)) of
        {LTLeft, empty} -> {LTLeft, node(Data, empty, Right)};
        {LTLeft, LTRight} -> {LTLeft, node(Data, LTRight, Right)}
    end.
